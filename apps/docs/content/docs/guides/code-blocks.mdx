...
Scribex includes a `CodeBlockNode` that provides editable code blocks with a language selector, copy button, and optional Shiki-powered syntax highlighting.
...
```tsx
import { $createCodeBlockNode } from "@scribex/core";
...
```tsx
import { $isCodeBlockNode } from "@scribex/core";
