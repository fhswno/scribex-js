...
description: How Scribex adapts to touch devices.
---

Scribex automatically adapts its UI for touch devices. Desktop-only features like drag handles are disabled, and a mobile-optimized formatting toolbar appears at the bottom of the screen. This happens transparently -- no configuration is needed.

## Touch detection

Scribex detects touch devices on mount using multiple signals for reliability:
...
import { EditorRoot, MobileToolbar, FloatingToolbar } from '@scribex/core';
...
The toolbar uses the same frosted glass aesthetic as other Scribex popovers:

- `backdrop-filter: blur(20px) saturate(180%)`
- Background: `var(--scribex-popover-bg)`
- Border: `var(--scribex-popover-border)` (top border only)
...
test('mobile toolbar appears on text selection', async ({ page }) => {
  ...
  // Type some text
  const editor = page.locator('[data-scribex-root]');
  await editor.click();
