...
Scribex includes a `CalloutPlugin` that provides Notion-style callout blocks. Each callout has a customizable emoji icon and a color preset that controls its background and border accent. A floating toolbar appears when the cursor is inside a callout, allowing quick changes to the emoji and color.
...
- `--scribex-callout-{id}-bg` -- the callout's background color
- `--scribex-callout-{id}-border` -- the callout's left border accent color

## Default presets

Scribex ships with six built-in presets:
...
```tsx
import { DEFAULT_CALLOUT_PRESETS } from "@scribex/core";
...
:root {
  --scribex-callout-tip-bg: #e8f5e9;
  --scribex-callout-tip-border: #4caf50;
  --scribex-callout-danger-bg: #ffebee;
  --scribex-callout-danger-border: #f44336;
  --scribex-callout-note-bg: #e3f2fd;
  --scribex-callout-note-border: #2196f3;
}
```
...
```tsx
import { $createCalloutNode } from "@scribex/core";
...
```tsx
import { $isCalloutNode } from "@scribex/core";
...
:root {
  /* Default preset */
  --scribex-callout-default-bg: #f1f1ef;
  --scribex-callout-default-border: #d4d4d4;
  ...
  /* Error preset */
  --scribex-callout-error-bg: #fbe4e4;
  --scribex-callout-error-border: #e03e3e;

  /* Success preset */
  --scribex-callout-success-bg: #ddedea;
  --scribex-callout-success-border: #0f7b6c;

  /* Purple / Note preset */
  --scribex-callout-purple-bg: #eae4f2;
  --scribex-callout-purple-border: #6940a5;
}
```
