...
Scribex provides a complete image upload pipeline with optimistic UI. When a user drops or pastes an image, they see an instant preview with a loading spinner. Once the upload completes, the preview is replaced with the permanent image. No base64 encoding is ever used.
...
Scribex provides no default implementation. You provide your own upload logic as a function. This keeps the library decoupled from any specific storage backend.
...
It renders a responsive `<img>` element and supports click-to-select with a selection ring (using the `--scribex-ring` token).

### Creating an image node programmatically

```tsx
import { $createImageNode } from "@scribex/core";
...
```tsx
import { $isImageNode } from "@scribex/core";
