...
Blokhaus never uses module-level mutable state or global singletons to track "the active editor." All state is scoped through React context provided by `LexicalComposer`. This is a strict architectural rule -- see the [project architecture](/docs/project-structure) for details.
