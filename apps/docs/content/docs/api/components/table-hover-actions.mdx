...
```tsx
import { TableHoverActions } from "@scribex/core";
```
...
The row and column context menus use the same popover styling as other Scribex menus:
...
- `--scribex-destructive` -- destructive action text color
- `--scribex-hover-bg` -- item hover background
