...
```tsx
import { FontPicker } from "@scribex/core";
...
| ------- | --------------------------- | --------------------------- |
| Default | (removes font override)     | `var(--scribex-font-sans)`  |
...
- The active font is highlighted with a subtle background color (`--scribex-hover-bg`).
- A checkmark icon appears to the right of the active font label, colored with `--scribex-accent`.
...
- Font values use CSS variable references by default (e.g., `var(--scribex-font-serif)`) so that the actual font stacks can be customized via `tokens.css` without modifying the picker.
