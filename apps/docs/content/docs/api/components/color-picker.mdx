...
import type { ColorPalette, ColorEntry } from "@scribex/core";
```
...
- Color values use CSS variable references (e.g., `var(--scribex-text-red)`) so that colors can be customized via the theme's `tokens.css` without modifying the picker.
