...
<Callout type="warn">
The `AIPreviewNode` must never call `editor.update()` during streaming. All streamed tokens are held in local React state (`useState`). This protects the undo history from being polluted with intermediate streaming states.
</Callout>

<Callout type="info">
After accepting AI content, a single `Cmd+Z` undo removes all accepted content in one step -- not token by token.
</Callout>
