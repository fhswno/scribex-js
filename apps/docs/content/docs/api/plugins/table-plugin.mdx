...
```tsx
import { TablePlugin } from '@scribex/core';
```

## Overview

The `TablePlugin` provides table creation, cell editing, row/column management, and optional cell merging. It wraps Lexical's `@lexical/table` module and registers the `INSERT_TABLE_COMMAND_SCRIBEX` command for creating tables with a specified number of rows and columns. The plugin handles keyboard navigation between cells using Tab and arrow keys.
...
import { EditorRoot, TablePlugin, InputRulePlugin, SlashMenu } from '@scribex/core';
...
|---|---|---|
| `INSERT_TABLE_COMMAND_SCRIBEX` | `{ rows: number; columns: number }` | Inserts a new table at the current cursor position with the specified dimensions. |

### Creating a table programmatically

```tsx
import { INSERT_TABLE_COMMAND_SCRIBEX } from '@scribex/core';
