...
<Callout type="info">
  The `ListPlugin` wraps `@lexical/list` and registers the necessary list nodes
  (`ListNode` and `ListItemNode`) automatically. You do not need to add them to
  the `ALL_NODES` array manually.
</Callout>

<Callout type="info">
  Pressing Enter on an empty list item at any nesting level first outdents the
  item. Only when the item is at the top level and empty does Enter exit the
  list entirely. This matches the behavior of editors like Notion and Google
  Docs.
</Callout>
