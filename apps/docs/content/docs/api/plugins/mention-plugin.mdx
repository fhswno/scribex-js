...
const userProvider = {
  ...
  onSearch: async (query: string) => {
    ...
    return users.map((user: { id: string; name: string; email: string; avatar: string }) => ({
      id: user.id,
      label: user.name,
      meta: user.email,
      icon: user.avatar,
  ...
  renderItem: (item) => (
    <div className="flex items-center gap-2">
      {item.icon && <img src={item.icon} alt="" className="w-5 h-5 rounded-full" />}
      <div>
  ...
  onSelect: (item) => $createMentionNode({
    id: item.id,
    label: item.label,
    trigger: '@',
  }),
...
<Callout type="warn">
Keep `renderItem` lightweight. This function is called on every keystroke while the dropdown is open. Avoid expensive computations or heavy DOM trees inside the render callback.
</Callout>
