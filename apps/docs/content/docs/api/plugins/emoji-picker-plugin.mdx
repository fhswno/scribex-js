  ...
  renderItem={(item) => (
    <div className="flex items-center gap-3 px-2 py-1">
      ...
      <div>
        ...
        {item.category && (
          <div className="text-xs text-scribex-muted-foreground">
            {item.category}
          </div>
        )}
      </div>
...
<Callout type="info">
  The colon trigger requires at least 2 characters after `:` before the dropdown
  appears. This prevents the picker from opening immediately on every colon
  character, which would interfere with normal writing.
</Callout>

<Callout type="info">
  When providing a custom `emojis` array, it completely replaces the built-in
  set. If you want to extend the built-in set, import `DEFAULT_EMOJIS` from
