...
<Callout type="info">
  The marquee selection is only active when the initial click lands in the
  gutter area (outside the text content). Clicking and dragging within text
  content performs a normal text selection, as expected.
</Callout>
...
<Callout type="warn">
  Block selection operates on top-level block nodes only. Nested content (such
  as list items inside a list or paragraphs inside a toggle) cannot be
  individually selected via the marquee. The entire parent block is selected.
</Callout>
